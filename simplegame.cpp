...
void SimpleGame::CpuMove()
{
    ...
    } else
    {
        ...
        // Make a move (We can make multiple moves in a row here, so do everything in the potentialMatches, THEN a random move)
        if(potentialMatches.size() > 0)
        {
            for(auto &pair : potentialMatches)
            {
                int x = pair.first;
                int y = pair.second;
                MakeMove(x, y);
